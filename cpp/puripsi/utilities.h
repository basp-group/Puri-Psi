#ifndef PURIPSI_UTILITIES_H
#define PURIPSI_UTILITIES_H
...
struct vis_params {
  ...
  vis_params(const Vector<t_real> &u_, const Vector<t_real> &v_, const Vector<t_real> &w_,
             ...
             const vis_units units_ = vis_units::lambda, const t_real &ra_ = 0, const t_real &dec_ = 0,
...
//! Generate guassianly distributed (sigma = pi) antenna positions
 Matrix<t_real> generate_antennas(const t_uint N, const bool testing = false);
...
//! A function that calculates the l2 ball radius for psi
...
Vector<t_complex>
add_noise(const Vector<t_complex> &y, const t_complex &mean, const t_real &standard_deviation, const bool testing = false);
...
bool parse_true_false_parameter(const std::string parameter, const std::string arg_name);
} // namespace utilities
} // namespace puripsi
