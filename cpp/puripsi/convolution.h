#ifndef PURIPSI_CONVOLUTION_H
#define PURIPSI_CONVOLUTION_H
...
template <class T>
inline Vector<T> zero_pad(const Vector<T> &input, const t_int padding);
...
template <class T>
Matrix<T> linear_convol_2d(const Vector<T> &kernelfu, const Vector<T> &kernelfv,
                           const Matrix<T> &kernelg);
//! perform linear convolution between two separable kernels and a 2d kernel
template <class T>
Matrix<T> linear_convol_2d(const std::function<T(t_int)> &kernelu,
