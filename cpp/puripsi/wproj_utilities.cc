...
Sparse<t_complex> create_chirp_row(const t_real &w_rate, const t_real &cell_x, const t_real &cell_y,
                                   ...
                                   const psi::OperatorFunction<Vector<t_complex>> &fftop) {
...
Sparse<t_complex> wprojection_matrix(const Sparse<t_complex> &G, const t_int &Nx, const t_int &Ny,
  ...
  PURIPSI_HIGH_LOG("Hard-thresholding of the Chirp kernels: energy [{}] ", energy_fraction_chirp);
  PURIPSI_HIGH_LOG("Hard-thresholding of the rows of G: energy [{}]  ", energy_fraction_wproj);

  const auto ft_plan = operators::fftw_plan::measure;
  const auto fftop_ = operators::init_FFT_2d<Vector<t_complex>>(Ny, Nx, 1., ft_plan);
  Sparse<t_complex> GW(Nvis, Npix);

  t_uint counts = 0;
  t_uint total_non_zero = 0;
#ifndef PURIPSI_OPENMP_FFTW
#pragma omp parallel for
  ...
  for(t_int m = 0; m < G.rows(); m++) {
    ...
    const Sparse<t_complex> chirp =
      create_chirp_row(w_components(m), cell_x, cell_y, Nx, Ny, energy_fraction_chirp, 
			 std::get<0>(fftop_));
    ...
    if(counts % 100 == 0) {
      PURIPSI_DEBUG("Row {} of {}", counts, GW.rows());
      PURIPSI_DEBUG("With {} entries non zero, which is {} entries per a row.", total_non_zero,
                   static_cast<t_real>(total_non_zero) / counts);
    }
  }
  assert(GW.nonZeros() > 0);
  PURIPSI_DEBUG("\nBuilding the rows of GW.. DONE!\n");
  PURIPSI_DEBUG("DONE - With {} entries non zero, which is {} entries per a row.", GW.nonZeros(),
