...
RMOperator::RMOperator(const utilities::rm_params &rm_vis_input, const t_int &Ju,
  ...
  */
  ...
  PURIPSI_HIGH_LOG("Constructing RM Operator");

  PURIPSI_MEDIUM_LOG("Oversampling Factor: %f", oversample_factor);
  std::function<t_real(t_real)> kernelu;

  std::function<t_real(t_real)> ftkernelu;

  PURIPSI_MEDIUM_LOG("Kernel Name: %s", kernel_name.c_str());
  PURIPSI_MEDIUM_LOG("Number of visibilities: %ld", rm_vis.frequency.size());
  PURIPSI_MEDIUM_LOG("Ju: %d", Ju);
  ...
  if(kernel_name == "kb_interp") {
    ...
    PURIPSI_DEBUG("Calculating weights");
    W = RMOperator::init_weights(rm_vis.frequency, rm_vis.weights, oversample_factor,
                                 weighting_type, R);
    PURIPSI_DEBUG("Doing power method");
    norm = std::sqrt(RMOperator::power_method(norm_iterations));
    PURIPSI_DEBUG("Gridding Operator Constructed");
    return;
  }

  if((kernel_name == "pswf") and (Ju != 6)) {
    PURIPSI_ERROR("Error: Only a support of 6 is implimented for PSWFs.");
  ...
  PURIPSI_DEBUG("Doing power method");
  norm = std::sqrt(RMOperator::power_method(norm_iterations));
  PURIPSI_DEBUG("Found a norm of %f", norm);
  PURIPSI_DEBUG("Gridding Operator Constructed");
}
}
