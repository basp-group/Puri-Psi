...
template <class T>
psi::OperatorFunction<T> init_gridding_degridding_matrix_2d(
  ...
  const Sparse<t_complex> interpolation_matrix = 
    details::init_gridding_matrix_2d(
				     u, v, w, weights, imsizey_, imsizex_, oversample_ratio, kernelu, 
				     kernelv, kernelw, Ju, Jv, Jw, 
				     w_term);
...
init_grid_degrid_operator_2d(const Vector<t_real> &u, const Vector<t_real> &v,
  ...
  const psi::OperatorFunction<T> phiTphi = puripsi::operators::base_grid_degrid_operator_2d<T>(
