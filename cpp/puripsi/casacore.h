#ifndef PURIPSI_CASACORE_H
#define PURIPSI_CASACORE_H
...
#define PURIPSI_MACRO(NAME, TYPE)                                                                   \
		case ::casacore::Tp##NAME:                                                                       \
		return details::get_taql_scalar<::casacore::TYPE>(vtable).template cast<T>();
	PURIPSI_MACRO(Complex, Complex);
	PURIPSI_MACRO(DComplex, DComplex);
#undef PURIPSI_MACRO
#define PURIPSI_MACRO(NAME, TYPE)                                                                   \
...
#define PURIPSI_MACRO(NAME, TYPE)                                                                   \
		case ::casacore::Tp##NAME:                                                                       \
		return details::get_taql_scalar<::casacore::TYPE>(vtable).template cast<T>();
	PURIPSI_MACRO(Bool, Bool);
	PURIPSI_MACRO(Char, Char);
	PURIPSI_MACRO(UChar, uChar);
	PURIPSI_MACRO(Short, Short);
	PURIPSI_MACRO(UShort, uShort);
	PURIPSI_MACRO(Int, Int);
	PURIPSI_MACRO(UInt, uInt);
	PURIPSI_MACRO(Float, Float);
	PURIPSI_MACRO(Double, Double);
#undef PURIPSI_MACRO
#define PURIPSI_MACRO(NAME, TYPE)                                                                   \
		case ::casacore::TpArray##NAME:                                                                  \
		return details::get_taql_array<::casacore::TYPE>(vtable).template cast<T>();
...
table_column(::casacore::Table const &table, std::string const &column, std::string const &filter) {
	return details::table_column<T>(table, column, filter,
			std::integral_constant<bool, psi::is_complex<T>::value>());
}
...
#define PURIPSI_MACRO(NAME)                                                                        \
		...
		Vector<t_complex> NAME(std::string const &col = "DATA") const {                                  \
			...
			Vector<t_complex> I = (weights.col(0).array().cast<t_complex>()*data.col(0).array() +          \
					weights.col(3).array().cast<t_complex>()*data.col(3).array())              \
					/(weights.col(0).array() + weights.col(3).array()).cast<t_complex>();      \
					return I;                                                                                      \
		}                                                                                                \
		...
		Vector<t_real> w##NAME(Sigma const &col = Sigma::OVERALL) const {                                \
			Matrix<t_real> weights = table_column<t_real>(ms_.table(), "WEIGHT", filter());                \
			Vector<t_real> w = (weights.col(3) + weights.col(0)).array().sqrt().cwiseInverse();          \
			/* the last /2 comes to counter-act a weird 2 factor in the .cc...   */                        \
			return w;                                                                                      \
		}                                                                                                \
		// // Standard deviation for the Stokes component [TO BE IMPROVED LATER ON!]
		// Vector<t_real> w##NAME(Sigma const &col = Sigma::OVERALL) const {
		//   Matrix<t_real> sigma = table_column<t_real>(ms_.table(), "WEIGHT", filter());
		//   Vector<t_real> w = (1./(sigma.col(3).array().square()) +
		//                         + 1./(sigma.col(3).array().square())).sqrt().cwiseInverse()/2/2;
		//   /* the last /2 comes to counter-act a weird 2 factor in the .cc...   */
		// }
		// Stokes parameters
		PURIPSI_MACRO(I);
...
#undef PURIPSI_MACRO
		...
		// }
		// /** Standard deviation for the Stokes component */  [in l.259] s
		// Vector<t_real> w##NAME(Sigma const &col = Sigma::OVERALL) const {
		...
		// /** Stokes component */
		// Vector<t_complex> I(std::string const &col = "DATA") const;
		// /** Standard deviation for the Stokes component [TO BE IMPROVED LATER ON!]*/
		// Vector<t_real> wI(Sigma const &col = Sigma::OVERALL) const;
		...
		bool is_valid() const;
		//! Extract flags associated with the channel of interest ([M, 4] (for the 4 correlations))
		Vector<bool> flag() const;
		//! Extract flag row [M, 1]
		Vector<bool> flag_row() const;
		//! Extract final flag (flag | flag_row) [M, 1]
		Vector<bool> final_flag() const;
...
//! Read measurement set object into vis_params structure
utilities::vis_params read_measurementset(MeasurementSet const &ms_file,
		const t_int channel_index,
		const t_real average_frequency,
		const MeasurementSet::ChannelWrapper::polarization pol
		= MeasurementSet::ChannelWrapper::polarization::I,
		std::string const &filter = "");
