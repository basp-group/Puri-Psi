...
Vector<t_complex> MeasurementOperator::preconditioned_degrid(const Image<t_complex> &eigen_image, const Vector<t_real> &preconditioner) const {
	 ...
	 */
	Matrix<t_complex> ft_vector = FFT(eigen_image);
	Vector<t_complex> result = G_function(ft_vector);
	result = ((preconditioner.array().sqrt().cast<t_complex>()) * result.array()).eval();
	return result;
}
...
Image<t_complex> MeasurementOperator::grid(const Vector<t_complex> &visibilities) const {
    ...
    st:: gridding parameters
	 */
	Matrix<t_complex> ft_vector = G_function_adjoint(visibilities);
	return inverse_FFT(ft_vector);
}

Image<t_complex> MeasurementOperator::preconditioned_grid(const Vector<t_complex> &visibilities, const Vector<t_real> &preconditioner) const {
	/*
    An operator that degrids an image and returns a vector of visibilities.
    visibilities:: input visibilities to be gridded
    st:: gridding parameters
	 */
	Matrix<t_complex> ft_vector = G_function_adjoint((visibilities.array() * (preconditioner.array().sqrt().cast<t_complex>())).eval());

	return inverse_FFT(ft_vector);
}

Vector<t_complex> MeasurementOperator::G_function(const Matrix<t_complex> &ft_vector) const{

	return utilities::sparse_multiply_matrix(G, ft_vector).array()*W;
}
...
Vector<t_complex> MeasurementOperator::G_function_adjoint(const Vector<t_complex> &visibilities) const{
	return G.adjoint() * (visibilities.array() * W.conjugate()).matrix();
}

Image<t_complex> MeasurementOperator::inverse_FFT(Matrix<t_complex> &ft_vector) const{

	ft_vector.resize(ftsizev_, ftsizeu_);
...
#pragma omp parallel for collapse(3)
	...
	index_vec.erase(unique(index_vec.begin(), index_vec.end()), index_vec.end());
	fourier_indices = Vector<t_int>::Map(&index_vec[0], index_vec.size());
...
MeasurementOperator::init_correction2d_fft(const std::function<t_real(t_real)> kernelu,
	...
	t_int x_start = std::floor(ftsizeu_ * 0.5 - imsizex() * 0.5);
	t_int y_start = std::floor(ftsizev_ * 0.5 - imsizey() * 0.5);
	Image<t_real> S = ftsizeu_*ftsizev_*fftoperator_.inverse(K).array().real().block(y_start, x_start, imsizey(),
			imsizex()); // probably really slow!
	return 1 / S;
}
...
MeasurementOperator::MeasurementOperator(
		const utilities::vis_params &uv_vis_input, Vector<t_real> const &preconditioner, const t_int &Ju, const t_int &Jv,
...
void MeasurementOperator::init_operator(const utilities::vis_params &uv_vis_input) {
	...
	ftsizeu_ = floor(imsizex() * oversample_factor());
	ftsizev_ = floor(imsizey() * oversample_factor());
	...
	if((kernel_name_ == "pswf") and (Ju_ != 6 or Jv_ != 6)) {
		PURIPSI_ERROR("Error: Only a support of 6 is implemented for PSWFs.");
	...
	if(kernel_name_ == "kb_min") {
		...
		= constant::pi * std::sqrt(Jv_ * Jv_ / (oversample_factor() * oversample_factor())
				* (oversample_factor() - 0.5) * (oversample_factor() - 0.5)
				- 0.8);
...
psi::LinearTransform<psi::Vector<psi::t_complex>>
puripsi::MeasurementOperator::linear_transform(t_uint nvis, t_int imsizey, t_int imsizex, t_int oversample_factor) {
	auto const height = imsizey;
	auto const width = imsizex;
	auto direct = [this, width, height](Vector<t_complex> &out, Vector<t_complex> const &x) {
		assert(x.size() == width * height);
		auto const image = Image<t_complex>::Map(x.data(), height, width);
		out = degrid(image);
	};
	auto adjoint
	= [this, width, height](Vector<t_complex> &out, Vector<t_complex> const &x) {
...
psi::LinearTransform<psi::Vector<psi::t_complex>>
puripsi::MeasurementOperator::linear_transform(t_uint nvis, Vector<t_real> const &preconditioner) {
	return linear_transform(nvis, imsizey(), imsizex(), oversample_factor(), preconditioner);
}

psi::LinearTransform<psi::Vector<psi::t_complex>>
puripsi::MeasurementOperator::linear_transform(t_uint nvis, t_int imsizey, t_int imsizex, t_int oversample_factor, Vector<t_real> const &preconditioner) {
	auto const height = imsizey;
	auto const width = imsizex;
...
psi::LinearTransform<psi::Vector<psi::t_complex>>
linear_transform(std::shared_ptr<MeasurementOperator const> const &measurements, t_uint nvis, Vector<t_real> const &preconditioner) {
	auto const height = measurements->imsizey();
	auto const width = measurements->imsizex();
