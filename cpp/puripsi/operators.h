...
template <class T>
t_real power_method(const psi::LinearTransform<T> &op, const t_uint &niters,
...
template <class T>
std::tuple<psi::OperatorFunction<T>, psi::OperatorFunction<T>>
base_padding_and_FFT_2d(const std::function<t_real(t_real)> &kernelu,
                        const std::function<t_real(t_real)> &kernelv, 
  ...
  PURIPSI_MEDIUM_LOG("Image size (width, height): {} x {}", imsizex, imsizey);
  PURIPSI_MEDIUM_LOG("Oversampling Factor: {}", oversample_ratio);
  std::tie(directZ, indirectZ) = puripsi::operators::init_zero_padding_2d<T>(S, oversample_ratio);
  ...
  std::tie(directFFT, indirectFFT)
      = puripsi::operators::init_FFT_2d<T>(imsizey, imsizex, oversample_ratio, ft_plan);
  auto direct = psi::chained_operators<T>(directFFT, directZ);
  auto indirect = psi::chained_operators<T>(indirectZ, indirectFFT);
  return std::make_tuple(direct, indirect);
}
...
template <class T>
std::shared_ptr<psi::LinearTransform<T> const>
init_degrid_operator_2d(const Vector<t_real> &u, const Vector<t_real> &v, const Vector<t_real> &w,
  ...
  // auto operator_norm = puripsi::operators::init_normalise<T>(op_norm);
  // direct = psi::chained_operators<T>(direct, operator_norm);
  // indirect = psi::chained_operators<T>(operator_norm, indirect);
  return std::make_shared<psi::LinearTransform<T> const>(direct, M, indirect, N);
}
...
template <class T, class... ARGS>
std::shared_ptr<psi::LinearTransform<T> const>
init_combine_operators(const std::vector<std::shared_ptr<psi::LinearTransform<T>>> &measure_op,
  ...
  return std::make_shared<psi::LinearTransform<T> const>(direct, indirect);
}

//! combines different uv_data sets and creates a super measurement operator out of multiple
//! measurement operators
template <class T, class... ARGS>
std::shared_ptr<psi::LinearTransform<T> const> init_super_operator(
    const std::function<std::shared_ptr<psi::LinearTransform<T> const>> &create_operator,
    const std::vector<utilities::vis_params> &uv_vis_data, ARGS &&... args) {
  const t_uint blocks = uv_vis_data.size();
  std::vector<std::shared_ptr<psi::LinearTransform<T>>> measure_ops;
  ...
  if(measure_ops.size() != uv_vis_data.size()) {
    throw std::runtime_error("Number of groups and measurement operators does not match.");
  }
