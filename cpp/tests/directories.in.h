#ifndef PURIPSI_DATA_DIR_H
#define PURIPSI_DATA_DIR_H
...
inline std::string scratch_directory() {
  std::string dirName = "$ENV{HOME}/Scratch/puripsi/data";
  struct stat sb;
  if ( stat("$ENV{HOME}/Scratch",&sb)!=0 || !S_ISDIR(sb.st_mode) )
    mkdir("$ENV{HOME}/Scratch",S_IRWXU);
  if ( stat("$ENV{HOME}/Scratch/puripsi",&sb)!=0 || !S_ISDIR(sb.st_mode) )
    mkdir("$ENV{HOME}/Scratch/puripsi",S_IRWXU);
  if ( stat("$ENV{HOME}/Scratch/puripsi/data",&sb)!=0 || !S_ISDIR(sb.st_mode) )
    mkdir("$ENV{HOME}/Scratch/puripsi/data",S_IRWXU);
  return dirName;
}
//! Holds data and such in Scratch (useful for legion)
