...
std::string usage() {
	return "PuriPsi: A program that will reconstruct images from radio interferometers using the Primal Dual algorthm. "
			"\n\n"
			...
			"--logging_level: Determines the output logging level for psi and puripsi. (\"debug\" is "
			"the default) \n\n";
}
...
Params parse_cmdl(int argc, char **argv) {
	...
	while(1) {
		...
		case 'r':
			params.kernel = kernels::kernel_from_string.at(optarg);
			break;
		...
		case 't':
			params.psi_logging_level = optarg;
			break;
		...
		case '1':
			if(optarg == "measure"){
				params.fftw_plan = operators::fftw_plan::measure;
			}else if(optarg == "estimate"){
				params.fftw_plan = operators::fftw_plan::estimate;
			}
