...
void pd(const std::string & name, const Image<t_complex> & M31, const std::string & kernel, const t_int J,  utilities::vis_params & uv_data, const t_real sigma, const t_real cellsize, const bool wterm){
	...
	auto const measurements_transform =
			std::make_shared<psi::LinearTransform<psi::Vector<psi::t_complex>>>(puripsi::operators::MeasurementOperator<Vector<t_complex>, t_complex>(
...
int main(int, char **) {
	...
	auto const sky_measurements = std::make_shared<psi::LinearTransform<psi::Vector<psi::t_complex>>>(puripsi::operators::MeasurementOperator<Vector<t_complex>, t_complex>(
