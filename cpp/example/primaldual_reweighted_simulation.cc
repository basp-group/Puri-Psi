...
int main(int nargs, char const **args) {
  ...
  PURIPSI_MEDIUM_LOG("Epsilon {}", epsilon);
  ...
  auto reweighted = psi::algorithm::reweighted(pd).min_delta(min_delta).is_converged(
      psi::RelativeVariation<std::complex<t_real>>(1e-5));
  std::clock_t c_start = std::clock();
  auto diagnostic = reweighted();
  std::clock_t c_end = std::clock();

  Image<t_complex> image = Image<t_complex>::Map(diagnostic.algo.x.data(), sky_model.rows(), sky_model.cols());
